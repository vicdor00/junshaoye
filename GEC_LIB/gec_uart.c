					 	...
					 	)
{
	...
	if (USARTx == USART1)
	{
		...
		//USART1_TX   GPIOA.9
		p.GPIO_Pin = GPIO_Pin_9; //PA.9
		p.GPIO_Speed = GPIO_Speed_50MHz;
		p.GPIO_Mode = GPIO_Mode_AF_PP;	//复用推挽输出
		GPIO_Init(GPIOA, &p);//初始化GPIOA.9

		//USART1_RX	  GPIOA.10初始化
		p.GPIO_Pin = GPIO_Pin_10;//PA10
		p.GPIO_Mode = GPIO_Mode_IN_FLOATING;//浮空输入
		GPIO_Init(GPIOA, &p);//初始化GPIOA.10
	...
	else if (USARTx == USART2)
	{
		...
		p.GPIO_Pin = GPIO_Pin_3;//PA3
		p.GPIO_Mode = GPIO_Mode_IN_FLOATING;//浮空输入
		GPIO_Init(GPIOA, &p);//初始化GPIOA.3
		RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2, ENABLE);
